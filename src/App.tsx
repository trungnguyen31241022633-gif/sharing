...
import { Search, ExternalLink, X, BookOpen, GraduationCap, Code, Compass, Tag, CheckCircle2 } from 'lucide-react';
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 py-12 md:py-20 w-full">
        ...
        {filteredCourses.length === 0 && (
          ...
          >
            ...
            <p className="text-slate-500 font-medium italic">Không tìm thấy khóa học nào phù hợp với tìm kiếm của bạn.</p>
          </motion.div>
        )}
      </main>
        ...
        {selectedCourse && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg glass-card rounded-3xl overflow-hidden shadow-2xl bg-white"
            >
              <div className="p-8">
                <button 
                  onClick={() => setSelectedCourse(null)}
                    ...
                    <div className="space-y-3">
                      ...
                      {selectedCourse.subLinks.map((sub, i) => (
                        ...
                        >
                          ...
                          <ExternalLink size={18} className="text-slate-400 group-hover:text-purple-500 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                        </a>
                      ))}
                    </div>
                  )}
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 mb-6 mt-auto">
        {course.hashtags.map(tag => (
          <span 
            key={tag} 
            className="text-xs px-2 py-1 rounded-md bg-slate-100 text-slate-600 font-medium group-hover:bg-purple-50 group-hover:text-purple-600 transition-colors"
          >
            #{tag}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-100/60">
        ...
        <motion.div 
          initial={false}
          className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-purple-600 group-hover:text-white transition-all shadow-sm"
        >
          <Compass size={16} />
        </motion.div>
      </div>
